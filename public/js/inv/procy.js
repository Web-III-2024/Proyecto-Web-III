...
function createDynamicContent(data) {
  ...
  mainContent.appendChild(recommendationsTitle);
...
function loadComments(sortBy) {
  ...
  headerRow.innerHTML =
    "<th>Nombre</th><th>Rating</th><th>Comentario</th><th>Fecha</th>";
  ...
  query
    ...
    .then(function (querySnapshot) {
      commentsBody.innerHTML = ""; // Limpiar comentarios anteriores antes de cargar nuevos
      querySnapshot.forEach(function (doc) {
        ...
        // Creamos las estrellas de calificación y las agregamos a la celda
        ratingCell.innerHTML = createRatingStars(data.rating);
        commentCell.textContent = data.comentario;
        // Convertimos el Timestamp de Firebase a una fecha legible
        const date = data.Fecha.toDate
          ? data.Fecha.toDate()
          : new Date(data.Fecha);
        dateCell.textContent = date.toLocaleDateString("default", {
          year: "numeric",
          month: "long",
          day: "numeric",
          hour: "2-digit",
